...
#    `src/ops/` (cluster + embed numerical primitives + the file-backed
#    mmap spill backend) or `src/embed/fbank.rs` (NEON/SSE2/AVX2/AVX-512F
#    window-mul, power-spectrum, dot kernels from the torchaudio fbank
...
#    / `power_spectrum` ensures miri sees the scalar path. The cfg name
#    is carried verbatim from the `diarization` crate this core was
#    extracted from (see `Cargo.toml` `[lints]` check-cfg). Per-backend
#    direct unsafe-call tests in `embed::fbank::tests` (e.g.
#    `dot_neon_agrees_with_scalar_directly`) are filtered out under
#    force_scalar because they call the unsafe SIMD kernels directly —
#    miri only exercises the dispatcher / scratch / scalar paths.
...
# propagation, and TLS scratch capacity bookkeeping.
cargo miri test \
  --lib --target "$TARGET" \
  -- \
  ops:: \
